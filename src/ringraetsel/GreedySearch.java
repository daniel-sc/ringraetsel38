...
public class GreedySearch {
	...
	while (context.currentDiff > 0) {
	    ...
	    System.out.println("Differenz: " + context.currentDiff + ", Tiefe: " + tiefe + ", Züge bisher: "
		    + result.size());
	    final int currentDiff2 = context.currentDiff;
	    List<Integer> resultSuche = Tiefensuche.tiefensucheParallel(tiefe, start.getCopy(), new CheckResult<T>() {

		@Override
		public boolean checkResult(List<Integer> indexOfMoves, AbstractZustand<T> current) {
		    int diff = current.differenz(ziel, currentDiff2);

		    if (diff < currentDiff2) {
			return true;
		    }
		    return false;
		...
		System.out.println("CHECK-Diff: " + start.vergleichen(ziel, 38).size());
		context.currentDiff = start.differenz(ziel, context.currentDiff);
		// System.out.println(start);
		System.out.println("NEXT MOVES (start rechts): " + currentMoves);
		tiefe = 1;
	    } else {
